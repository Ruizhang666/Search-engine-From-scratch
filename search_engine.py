...
def compute_tfidf(doc, doc_freqs, weights):
    ...
    # convert the term counts into term frequency  
    term_frequency=compute_tf(doc, doc_freqs, weights)

    # compute the tf-idf

    out=defaultdict(float)

    for i in term_frequency.keys():
        tf=term_frequency[i]
        if doc_freqs[i]!=0:
            idf=1/doc_freqs[i]
        else:
...
def jaccard_sim(x, y):
    ...
    if num==0:
        return 0
    deno=(sum(list(x.values()))+sum(list(y.values())))-num
    if deno==0: 
        return 0
    return num/deno  
...
def overlap_sim(x, y):
    ...
    if num==0:
        return 0
    deno=min(sum(list(x.values())),sum(list(y.values())))
    if deno==0: 
        return 0
    return num/deno  
...
def precision_at(recall: float, results: List[int], relevant: List[int]) -> float:
    ...
    # compute the recall-precision pair 

    precision_list=[1]
    recall_list=[0]

    for i in range(len(relevant)):
        num_relavant_needed=i+1
        current_relavant=0
        total_retrived=0
        for j in range(len(results)):
            current_doc=results[j]
            if current_doc in relevant:
                current_relavant+=1
            total_retrived+=1
            if current_relavant==num_relavant_needed:
                break
        precision=current_relavant/total_retrived
        current_recall=num_relavant_needed/len(relevant)
        precision_list.append(precision)
        recall_list.append(current_recall)

    for i in range(len(recall_list)):
        if recall_list[i]==recall:
            return precision_list[i]

    for i in range(len(recall_list)):
        if i!= len(recall_list)-1:
            if recall_list[i]<recall and recall_list[i+1]>recall:
                return interpolate(recall_list[i],precision_list[i],recall_list[i+1],precision_list[i+1],recall)
...
def mean_precision2(results, relevant):
    ...
    for i in range(10):
        total+=precision_at((i+1)/10, results, relevant)
...
def norm_precision(results, relevant):
    ...
    denominator=len(results)*np.log(len(results))-(len(results)-len(relevant))*np.log((len(results)-len(relevant)))-len(relevant)*np.log(len(relevant))

    return 1- (sum_rank-sum_i)/denominator
